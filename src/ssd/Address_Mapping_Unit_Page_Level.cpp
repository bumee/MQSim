...
namespace SSD_Components
{
	...
	*/
					...
					for (unsigned int plane_cntr = 0; plane_cntr < domains[stream_id]->Plane_no; plane_cntr++) {
						...
						for (int valid_pages_in_block = pages_no_per_block; valid_pages_in_block >= 0; valid_pages_in_block--) {
						 ...
						 if (assigned_lpas[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID].size() > 0) {
							PRINT_ERROR(
							"Preconditioning failed on plane "
							<< "C" << plane_address.ChannelID << " Ch" << plane_address.ChipID << " D" << plane_address.DieID << " P" << plane_address.PlaneID
							<< ": unassigned LPAs = " << assigned_lpas[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID].size());
						 }
					}
				}
			}
	...
	{
		std::cout << "total logical pages: " << this->domains[stream_id]->Total_logical_pages_no << std::endl;
